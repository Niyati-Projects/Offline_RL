...
class QLearning:
    ...
    def state_to_index(self, state):
        return int(state[0] * self.env.grid_size[1] + state[1])
...
def parameter_analysis(env, param_name, param_values, num_episodes=1000):
    ...
    #     plt.plot(range(num_episodes), rewards, label=f"{param_name}={param_value}", linewidth= 1)
    # plt.xlabel("Episodes")
...
if __name__ == "__main__":
    ...
    # Parameter sensitivity analysis
    parameter_analysis(env, 'alpha', [0.09, 0.75, 0.96])
    #parameter_analysis(env, 'gamma', [0.5, 0.7, 0.9])
    #parameter_analysis(env, 'epsilon', [1.0, 0.5, 0.1])
    
    # Plot rewards for individual runs
    # plt.figure(figsize=(10, 6))
    # for i, rewards in enumerate(trials):
    #     plt.plot(rewards, label=f"Run {i + 1}")
    # 
    # plt.xlabel("Episodes")
    # plt.ylabel("Cumulative Reward")
    # plt.title("Cumulative Rewards for Individual Runs")
    # plt.legend()
    # plt.show()

# Aggregate results (mean and standard deviation)
    mean_rewards = np.mean(trials, axis=0)
    std_rewards = np.std(trials, axis=0)
...
# Plot mean with error bars
    ...
    trials = learn(env, 1)  # Rewards from 10 runs
    rewards_array = np.array(trials)

# Calculate percentiles
    percentile_25 = np.percentile(rewards_array, 25, axis=0)
    percentile_50 = np.percentile(rewards_array, 50, axis=0)  # Median
...
# Plot percentiles
    ...
    # plt.xlim(0, 1000)  
    # plt.ylim(-70, 150)
    # plt.plot(percentile_50, label="Median (50th Percentile)", color="blue", linewidth=1)
    # plt.fill_between(range(rewards_array.shape[1]),
    #              percentile_25, percentile_75, color="blue", alpha=0.2,
    #              label="25th-75th Percentile Range")
    # 
    # plt.xlabel("Episodes")
    # plt.ylabel("Cumulative Reward")
    # plt.title("Percentile Plot of Rewards Across Runs")
    # plt.legend()
    # plt.show()
